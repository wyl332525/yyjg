...
public class BitmapCache extends Activity {
	...
	public void displayBmp(final ImageView iv, final String thumbPath,
		...
		new Thread() {
			...
			public void run() {
				...
				if (thumb == null) {
					thumb = SelectFileActivity.bimap;
				}
	...
	public void getBitmapDelayer(final ImageView imageView, final String path,final ImageCallback callBack){
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					final Bitmap bmp = revitionImageSize(path);
					h.post(new Runnable() {
						
						@Override
						public void run() {
							// TODO Auto-generated method stub
							callBack.imageLoad(imageView, bmp, path);
						}
					});
				} catch (IOException e) {
