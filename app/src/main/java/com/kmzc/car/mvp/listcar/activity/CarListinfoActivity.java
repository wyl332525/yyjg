package com.kmzc.car.mvp.listcar.activity;
...
import com.kmzc.car.mvp.detailcar.activity.CarDetailActivity;
import com.kmzc.car.mvp.detailcar.model.EventBean;
import com.kmzc.car.mvp.listcar.adapter.CarListinfoAdapter;
import com.kmzc.car.mvp.listcar.contract.CarListInfoContract;
import com.kmzc.car.mvp.listcar.model.CarCheckBean;
import com.kmzc.car.mvp.listcar.model.CarListInfoModel;
import com.kmzc.car.mvp.listcar.presenter.CarListInfoPresenter;
import com.kmzc.car.mvp.login.model.LoginCarTypeBean;
import com.kmzc.car.mvp.searchcar.activity.CarSearchDetailActivity;
...
import com.kmzc.core.utils.SpUtil;
        ...
        lvCarinfo.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                if (title1.equals("修改检测方法") && type == 3) {
                    ...
                    Log.i("baixg:","3");
                    Log.i("baixg:",data.get(i).getRECHECKINFO());
                    if (data != null && data.size() > 0) {
                        Log.i("zhouh:",data.get(i).getCHECKMETHOD());
                        if(data.get(i).getCHECKMETHOD().equals("加载减速工况法")||data.get(i).getCHECKMETHOD().equals("自由加速法")){
                            Log.i("zhouh1:", title1);
                           if(title1.equals("待查验列表")) {
                               keys = getResources().getStringArray(R.array.checkdetai2);
                           }else{
                               keys = getResources().getStringArray(R.array.reciverdetai2);
                           }
                        } else {
                            if(title1.equals("待查验列表")) {
                                keys = getResources().getStringArray(R.array.checkdetail);
                            }else{
                                keys = getResources().getStringArray(R.array.reciverdetail);
                            }
                        }
                        ...
                        intent.putExtra("reCheckInfo", data.get(i).getRECHECKINFO());
                        intent.putExtra("checkMethod", data.get(i).getCHECKMETHOD());
                        startActivity(intent);
                    } else {
